...
public class ProbCalc {
	public static double CalculateWordProbability(String word, List<String> imageIDs,
		...
		double prob = getProb(word, imageIDs, modelType, len);
	...
	private static double getProb(String word, List<String> imageIDs,
		...
		case COMBINED_MODEL:
			for (int i = 0; i < len - 1; i++) {
				for (int j = i + 1; j < len; j++) {
					if (imageIDs.get(i) == imageIDs.get(j)) {
	...
	private static void calculateZ(ModelType modelType, int len, int idx,
		...
		if (idx == len) {
			z.Z += getProb(String.valueOf(wordArray), imageIDs, modelType, len);
			return;
		}
